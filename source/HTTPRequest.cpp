...
void HTTPUrlParser::parse()
{
    ...
    this->protocol_ = std::string( purl + protocol_begin, protocol_end - protocol_begin );
    this->host_ = std::string( purl + domain_pos_begin, domain_pos_end - domain_pos_begin );
    ...
    {
        this->path_ = std::string( purl + domain_pos_end, len - domain_pos_end );
    }
...
std::string HTTPRequest::build_header()
{
    ...
    context += this->method() + " " + url_parser_.path() + " HTTP/1.1\r\n";

    for ( auto itr = this->header_.begin(); itr != this->header_.end(); itr++ )
    {
        auto kv = *itr;
